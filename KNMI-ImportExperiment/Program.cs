...
namespace KNMI_ImportExperiment
{
  class Program
  {
    ...
    static async Task Main(string[] args)
    {
      ...
      //Get the to download data.
      const string connection = "Trusted_Connection=True;Data Source=(Local);Database=Weather;MultipleActiveResultSets=true";
      string stationsDownload;
      DateTime lastestDateDownload;

      using (Db = new WeatherDbContext(connection))
      {
        stationsDownload = GetStationsDownload();
        lastestDateDownload = GetLatestDateDownload();
      }
      ...
      data = await DownLoadData(stationsDownload, lastestDateDownload.AddDays(1).ToString("yyyyMMdd"));
      header = GetDownloadHeader(data);
    ...
    /// <summary>
    /// Get the station numbers to download.
    /// </summary>
    /// <returns></returns>
    private static string GetStationsDownload()
    {
    ...
    /// </summary>
    /// <returns></returns>
    private static DateTime GetLatestDateDownload()
    {
    ...
    /// <summary>
    /// Download the weather data from KNMI.
    /// </summary>
    /// <returns></returns>
    private static async Task<string> DownLoadData(string stations, string date)
    {
      ...
      HttpResponseMessage httpRespond =
        await http.GetAsync($"{url}?stns=260&start=20200101");
#else
      HttpResponseMessage httpRespond =
        await http.GetAsync($"{url}?stns={stations}&start={date}");
#endif
    ...
    private static void ProcessData(string data, string[] header)
    {
    ...
    private static void ProcessLine(string[] header, string[] fields)
    {
